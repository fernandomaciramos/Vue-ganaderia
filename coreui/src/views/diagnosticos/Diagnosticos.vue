<template>
  <CRow>
    <CCol col="12" lg="6">
      <CCard no-header>
        <CCardBody>
          <div :style="titulo" class="titulo">Ganado id:  {{ $route.params.id }}</div>
        ...
        <CDataTable
          ...
          <template #edit="{item}">
            <td>
              <CButton color="primary" @click="editDiagnostico( item.id )">Editar</CButton>
            </td>
          </template>
          <template #delete="{item}">
            <td>
              <CButton v-if="you!=item.id" color="danger" @click="deleteDiagnostico( item.id )">Eliminar</CButton>
            </td>
          </template>
        </CDataTable>
...
export default {
  ...
  */
  data: () => {
    return {
      items: [],
      fields: ['id','diagnostico', 'tratamiento', 'inicio', 'final','edit', 'delete'],  
      diagnostico: '',
      tratamiento:'',
      inicio:'',
      final:'', 
  ...
  methods: {
    ...
    editLink (id) {
      ...
      return `editdiagnostico/${id.toString()}`
    },
    ...
    deleteDiagnostico ( id ) {
      let self = this;
      let ganadoId = id;
      axios.post(  this.$apiAdress + '/api/diagnosticos/' + id + '?token=' + localStorage.getItem("api_token"), {
        _method: 'DELETE'
      })
      .then(function (response) {
          self.message = 'Diagnostico eliminado con exito.';
          self.showAlert();
          self.getDiagnostico();
    ...
    getDiagnostico(){
      let self = this;
      axios.get(  this.$apiAdress + '/api/diagnosticos/' + self.$route.params.id + '?token=' + localStorage.getItem("api_token"))
      .then(function (response) {
